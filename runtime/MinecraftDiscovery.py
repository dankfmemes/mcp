...
def getLibraries(root, jsonfile, osKeyword):
    ...
    for library in mcLibraries:
        ...
        if 'natives' in library:
            libFilename = "%s-%s-%s.jar" % (libSubdir,
                                            libVersion, library['natives'][osKeyword])
        else:
        ...
        libRelativePath = os.path.join(
            "libraries", libPath, libSubdir, libVersion, libFilename)
...
if __name__ == '__main__':
    ...
    mcLibraries = getLibraries(
        mcDir, getJSONFilename(mcDir, "1.6.1"), osKeyword)
